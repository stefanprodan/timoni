...
sidebarTitle: "Module specification"
description: "Structure, configuration schema, and metadata of a Timoni module."
---
...
```json
{
  ...
  "annotations": {
    ...
    "sh.timoni.images": "docker.io/nginxinc/nginx-unprivileged:1-alpine@sha256:c3fed6436b61d2bf2201ec032c35c000871f7ed062dea5d586bc6bf4d0fdd140"
  }
}
```
...
The module metadata is stored in the artifact manifest as annotations.
Timoni sets the following annotations when pushing a module:

| Annotation                          | Description                                                                 |
|-------------------------------------|-----------------------------------------------------------------------------|
| `org.opencontainers.image.version`  | The module version, set from the `--version` flag                           |
| `org.opencontainers.image.created`  | The artifact creation date in RFC 3339 format                               |
| `org.opencontainers.image.source`   | The Git repository URL of the module source, set when available             |
| `org.opencontainers.image.revision` | The Git commit SHA of the module source, set when available                 |
| `sh.timoni.images`                  | The container images declared in the module's `images.cue` file             |

The `sh.timoni.images` annotation records each image in the
...
```text
docker.io/curlimages/curl:8.21.0@sha256:7c12...,docker.io/nginx:1.31.4-alpine@sha256:db35...
```

Custom annotations, such as the module's license, description, and documentation URL,
can be added with the `--annotation` flag of `timoni mod push`:
...
A Timoni instance represents a module instantiation on a Kubernetes cluster.
...
please see the [CUE introduction](/cue/introduction).

To get familiar with the CUE syntax, please see the
[CUE guide](/cue/walkthrough). This guide will walk you through
...
- [Module initialization](/cue/module/initialization)
- [Add Kubernetes custom resources](/cue/module/custom-resources)
- [Cluster version constraints](/cue/module/semver-constraints)
- [Control the Apply Behavior](/cue/module/apply-behavior)
- [Run tests with Kubernetes Jobs](/cue/module/test-jobs)

## Module Distribution

Timoni modules are distributed as OCI artifacts, for more information please see:

- [Module OCI artifact format](/cue/module/publishing#artifact-format)
- [Module versioning](/cue/module/publishing#version-format)
- [Publishing module versions](/cue/module/publishing#publishing-module-versions)
- [Module signing and verification](/cue/module/signing)
