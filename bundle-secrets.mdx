---
title: "Bundle Secrets Injection"
sidebarTitle: "Bundle secrets injection"
description: "Inject secrets from Kubernetes, CI envs, 1Password or SOPS."
---

Ways of injecting secrets when deploying applications with Timoni [Bundles](/bundle):

- Using [runtime](#runtime-secrets) attributes `@timoni(runtime:string:SECRET-NAME)`.
- Using [1Password](#1password-secrets) secret references resolved by the `op` CLI.
...
Example of a Timoni [Bundle Runtime](/bundle-runtime) that fetches the secrets from the cluster:
...
When using [1Password](https://developer.1password.com/docs/cli/secrets-environment-variables/),
the `op run` command resolves secret references to env vars and passes them
to Timoni as a subprocess, where they are read with `--runtime-from-env`.

### Injecting secrets with op CLI

Using the bundle from the [CI example](#injecting-secrets-from-ci-secret-store),
create an env file `secrets.env` that maps the runtime attribute names
to 1Password secret references:

```shell
ACCESS_KEY=op://my-vault/my-app-storage/access-key
SECRET_KEY=op://my-vault/my-app-storage/secret-key
```

At apply-time, run Timoni with `op run`:

```shell
op run --env-file=secrets.env -- timoni bundle apply -f bundle.cue --runtime-from-env
```

The `op` CLI authenticates with the 1Password desktop app on workstations,
...
```shell
op run --env-file=secrets.env -- timoni bundle build -f bundle.cue --runtime-from-env
```
